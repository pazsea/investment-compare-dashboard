import { useState } from 'react'
import type { ChangeEvent, FC } from 'react'

import { useSearchInstrumentsQuery } from '../../api/instrumentsApi'
import { EmptyState } from '../../components/EmptyState'
import { ErrorState } from '../../components/ErrorState'
import { InstrumentCard } from '../../components/InstrumentCard'
import { LoadingState } from '../../components/LoadingState'
import { useDebounce } from '../../hooks/useDebounce'
import type { Instrument } from '../../types/instrument'

import * as styles from './SearchPage.css'

const MIN_QUERY_LENGTH = 2

const renderInstrumentCard = (instrument: Instrument) => {
  return <InstrumentCard key={instrument.symbol} instrument={instrument} />
}

const SearchPage: FC = () => {
  const [query, setQuery] = useState('')
  const debouncedQuery = useDebounce(query.trim(), 350)
  const shouldSearch = debouncedQuery.length >= MIN_QUERY_LENGTH
  const { data: instruments = [], isFetching, isError } = useSearchInstrumentsQuery(debouncedQuery, {
    skip: !shouldSearch,
  })

  const handleQueryChange = (event: ChangeEvent<HTMLInputElement>) => {
    setQuery(event.target.value)
  }

  const resultLabel = instruments.length === 1 ? 'träff' : 'träffar'

  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <header className={styles.header}>
          <p className={styles.eyebrow}>Instrumentsök</p>
          <h1 className={styles.title}>Hitta aktier, ETF:er, fonder och krypto.</h1>
          <p className={styles.summary}>
            Sök på namn eller symbol för att hitta instrument till framtida jämförelser och bevakning.
          </p>
        </header>

        <section className={styles.searchPanel} aria-label="Formulär för instrumentsök">
          <label className={styles.label} htmlFor="instrument-search">
            Sök instrument
          </label>
          <input
            id="instrument-search"
            className={styles.input}
            type="search"
            value={query}
            placeholder="Testa AAPL, VOO, Bitcoin eller Microsoft"
            autoComplete="off"
            onChange={handleQueryChange}
          />
          <p id="instrument-search-heading" className={styles.helper}>
            Skriv minst två tecken för att söka.
          </p>
        </section>

        {!shouldSearch && <EmptyState message="Börja med en symbol eller ett bolagsnamn." />}

        {shouldSearch && isFetching && (
          <LoadingState
            title="Söker instrument"
            message="Matchande resultat visas här när de laddas in."
            variant="search-results"
          />
        )}

        {shouldSearch && isError && (
          <ErrorState message="Sökningen är inte tillgänglig just nu. Försök igen om en stund." />
        )}

        {shouldSearch && !isFetching && !isError && instruments.length === 0 && (
          <EmptyState message={`Inga instrument hittades för "${debouncedQuery}".`} />
        )}

        {shouldSearch && !isError && instruments.length > 0 && (
          <section aria-labelledby="search-results-heading">
            <div className={styles.resultsHeader}>
              <h2 id="search-results-heading" className={styles.resultsTitle}>
                Sökresultat
              </h2>
              <span className={styles.resultCount}>
                {instruments.length} {resultLabel}
              </span>
            </div>
            <div className={styles.grid}>{instruments.map(renderInstrumentCard)}</div>
          </section>
        )}
      </div>
    </main>
  )
}

export default SearchPage
